...
for (file in files) {
  ...
  # 按照起始和结束分组
  if (length(exons) > 0) { # 检查是否提取到外显子位置
...
# 提取所有外显子序列
all_start_sequences <- unlist(lapply(results, function(x) x$res_sequences[1, ]))
all_end_sequences <- unlist(lapply(results, function(x) x$res_sequences[2, ]))
